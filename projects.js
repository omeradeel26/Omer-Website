...
function disp_modal(info) {
  ...
  //retrieve info
  const { stack, features, github, date, video } = data[info].modal;

  document.getElementById("modalTitle").innerHTML = info;
  document.getElementById("modalGit").href = github;
  document.getElementById("modalDate").innerHTML = date;

  const DOMFeatures = document.getElementById("modalFeatures");
  DOMFeatures.innerHTML = "";

  features.forEach((feature) => {
    const li = document.createElement("li");
    li.innerHTML = feature;
    li.classList.add("modal__features");
    DOMFeatures.appendChild(li);
  });

  const DOMStack = document.getElementById("modalStack");
  DOMStack.innerHTML = "";
  ...
  stack.forEach((item) => {
    ...
    li.innerHTML = item;
    li.classList.add("modal__features");
    DOMStack.appendChild(li);
  });

  document.getElementById("modal_video").style.display = "block";
  document.getElementBId("modal_info").style.width = "40%";
...
function get_projects() {
  ...
  for (const [key, project] of Object.entries(data)) {
    //loop through each project
    for (type of Object.values(filter_log)) {
      //loop through the filter log to compare to the projects
      //get list of all filters on project
      if (type != "") {
        if (
          !Object.values(project.filter).includes(type) &&
          !unused.includes(project.name)
        ) {
